...
function AppointmentCardHeader({
  ...
  let subtitle = (
    <React.Fragment>
      {locations[appointment.location_id].name} &middot; {spareCapacity} slot
      {spareCapacity === 1 ? "" : "s"} left
    </React.Fragment>
  );

  let hiddenWarning = appointment.status === "hidden" && (
    <React.Fragment> &middot; HIDDEN</React.Fragment>
  );

  let description = isStaff && appointment.description && (
    <React.Fragment> &middot; {appointment.description}</React.Fragment>
  );
