function FutureSlots({ state }) {
  ...
  const hiddenTypes =
    currentUser && currentUser.isStaff ? ["resolved"] : ["resolved", "hidden"];

  const currentAppointments = appointments.filter(
    ({ status }) => !hiddenTypes.includes(status)
  );

  const [compact, setCompact] = React.useState(false);
  const [hideFull, setHideFull] = React.useState(false);

  React.useEffect(() => setCompact(currentUser && !currentUser.isStaff), [
      ...
      <div className="container">
        ...
        >
          ...
          <h5>
            ...
            {state.config.daily_appointment_limit} appointments per day, or more
            than {state.config.weekly_appointment_limit} per week. You also
            cannot be signed up for more than{" "}
            {state.config.simul_appointment_limit} appointments that have not
            yet occurred.
          </h5>
        </div>
        ...
        {Array.from(days.entries()).map(([day, dayAppointments]) => (
          <div>
            <h3>{day}</h3>
            {currentUser != null && currentUser.isStaff && (
              <AppointmentButtons ids={dayAppointments.map(({ id }) => id)} />
            )}
            <div
              className={
                "card-holder " + (compact ? "compact-card-holder" : "")
              }
            >
