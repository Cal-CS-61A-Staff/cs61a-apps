...
def cli(
    ...
    Simply run `bt` to build the default target in your project, installing dependencies
    as necessary. Run `bt TARGET1 TARGET2 ...` to build particular targets.
    """
    try:
        repo_root = find_root()
        os.chdir(repo_root)
        ...
        if not skip_setup:
            ...
            setup_targets = [
                target for target in targets if target.startswith("setup:")
            ]
