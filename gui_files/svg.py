...
class SVGCircle():
    ...
    def __str__(self):
        return """<circle cx="{0}" cy="{1}" r="{2}" stroke="{3}" fill="{4}" />""".format(
...
class SVGGraphic():
    ...
    def __str__(self):
        ...
        return """<svg width="{0}" height="{1}" xmlns="http://www.w3.org/2000/svg">{2}</svg>""".format(self.width, self.height, shapes)
